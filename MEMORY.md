...
| Next phase | **021 — Core Runtime Dependency Introduction.** Not started. It is the phase the lock gate is already waiting for: the moment `project.dependencies` stops being empty, `LOCK_RUNTIME_UNLOCKED` fails until `pylock.toml` accompanies it. It also inherits three things Phase 020 recorded as deferred with its name on them — whether the `dev` extra becomes a PEP 735 dependency group (measured as producing a byte-identical lock, so the argument is about publication rather than about the artefact), widening the SBOM from the declared set to the locked transitive set, and the severity threshold in `docs/DEPENDENCY_POLICY.md`, which names itself as the place that argument happens once something ships. |
