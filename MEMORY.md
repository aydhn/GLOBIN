...
| Next phase | **025 — TA-Lib Native Library Provisioning.** Not started. It inherits a harness that can measure whether a workload benefits, a health surface that can say what this process is consuming, and a support bundle that can carry evidence off the machine safely. `docs/engineering/WHEEL_AVAILABILITY.md` files `ta-lib` under phase 25 and records that the Python wrapper needs a native library this repository has not provisioned; nothing about that native dependency has been measured on this host. |
| Roadmap | [`ROADMAP.md`](ROADMAP.md); band skeleton in `src/globin/roadmap.py` |

**The roadmap has been amended eight times.** Band ranges, phase numbers and band
width are unchanged by all six; amending phase scope requires an ADR. The fourth
