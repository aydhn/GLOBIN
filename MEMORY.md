...
| Next phase | **030 -- Bootstrap Health Check Suite.** Not started. It inherits a registry of **eighteen** checks in `globin.domain.bootstrap.checks()`, which Phase 021 wrote as a function so that later phases add to it rather than rewrite it -- Phase 029 added the eighteenth through that extension point and proposed no suite, no scheduling and no periodicity, all of which are 030's. What it should read first is ADR-0076's refusal-boundary section, which names the observable signal that this phase drew the line wrong: 030 finding it must argue with `checks()` rather than extend it. **Phase 031's collision is not refused and says so.** The offline materialization gate answers a substantial part of *Offline and Degraded Installation Handling*; what is left to 031 is the runtime *behaviour* under degradation rather than a verdict about a lock and a directory. |
| Roadmap | [`ROADMAP.md`](ROADMAP.md); band skeleton in `src/globin/roadmap.py` |

**The roadmap has been amended thirteen times.** Band ranges, phase numbers and band
width are unchanged by all ten; amending phase scope requires an ADR. The fourth
