...
| Next phase | **031 -- Offline and Degraded Installation Handling.** Not started. Phase 029's materialization gate already answers a substantial part of it -- whether the committed lock could be installed from local bytes with no network -- so what is left is the runtime *behaviour* under degradation rather than a verdict about a lock and a directory. It inherits from Phase 030 a classification it should read before adding a check: a check that cannot be measured because something is absent is `UNMEASURED`, which is never a pass, and a capability the registry *predicted* absent must not make a host amber. **Phase 032 is two phases away and is holding the granularity review**, now with fourteen amendments in front of it; ADR-0079 adds a data point to it rather than an argument. |
