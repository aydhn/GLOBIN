...
| Next phase | **023 — NVIDIA Driver and CUDA Capability Detection.** Not started. It inherits a verified numerical stack that nothing imports, and a runtime tree it may write evidence into. `docs/engineering/BOOTSTRAP.md` carries the handoff and `docs/engineering/RUNTIME_FILESYSTEM.md` the tree's contract; neither GPU presence nor driver version has been measured, and `wheel-survey.toml` says only that CUDA builds are served from a separate index. |
| Roadmap | [`ROADMAP.md`](ROADMAP.md); band skeleton in `src/globin/roadmap.py` |

**The roadmap has been amended six times.** Band ranges, phase numbers and band
