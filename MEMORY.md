...
| Next phase | **026 — Configuration File Layout and Profiles.** Not started. It inherits a typed configuration model with three sections and twenty-two settings that is fed by **no source at all** — `build_configuration` passes none, so production runs on declared defaults. `docs/CONFIGURATION_POLICY.md` records that nothing in the model knows about files, and that the `config/` directory does not exist because creating it would settle this phase's question by accident. |
