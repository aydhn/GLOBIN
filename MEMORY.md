...
| Next phase | **029 -- Credential Prompting and Validation Flow.** Not started. It inherits a store that works and holds nothing: `StoreBackedSecrets.required` is empty, and filling it in is 029's. `getpass` documents a fallback that **echoes** and signals it with `GetPassWarning`; `SECRET_STORE_CONTRACT.md` section 5 requires that warning to **abort collection** rather than be printed. The absent-capability rule is now narrower rather than gone -- `credential`, `password`, `token`, `keyring` and `apikey` are still forbidden as module names, and 029 is the phase that lifts the first. |
| Roadmap | [`ROADMAP.md`](ROADMAP.md); band skeleton in `src/globin/roadmap.py` |

**The roadmap has been amended twelve times.** Band ranges, phase numbers and band
