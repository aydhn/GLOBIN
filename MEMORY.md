...
| Next phase | **027 -- Environment Variable and Profile Resolution.** Not started. It inherits a layout that *computes* candidate documents and deliberately refuses to order them: `ConfigLayout.documents_for` returns a mapping precisely so that precedence is still open. `build_configuration` still passes no sources. |
| Roadmap | [`ROADMAP.md`](ROADMAP.md); band skeleton in `src/globin/roadmap.py` |

**The roadmap has been amended ten times.** Band ranges, phase numbers and band
